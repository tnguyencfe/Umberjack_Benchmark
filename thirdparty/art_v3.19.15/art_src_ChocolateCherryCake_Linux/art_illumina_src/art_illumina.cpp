		...
		while(seq_reader.next_seq(id,a_art.ref_seq)){
			...
			while(t_num_read>0){
				...
				if(sam_out){
					...
					// The pos field of SAM records is always with respect to the  leftmost coordinate of the forward direction reference sequence.
					// When the read is longer than the fragment, we introduce adapter contamination on the 3' end of each read.
					// If the read is longer than the fragment size, then the reverse direction read will overhang the fragment at the 3' end of the reverse direction,
					// which is also the 5' end of the forward direction.
					//           pos forward read
					//           |
					//           |fragment|adapter->
					//  <-adapter|fragment|
					...
					//           pos reverse read
					//
					// For the pos field of the reverse read, We must choose the position wrt forward direction reference sequence that is larger,
					// whether the read length is larger than the fragment size or not.
					//
					//   pos forward read
					//   |------------>
					//                <-------|
					...
					if(a_read.is_plus_strand){
						...
						sR2.pos=max(long(a_art.ref_seq.size()-(a_read_2.bpos+read_len-1)), sR.pos) ;
						sR2.reverse_comp();
					}
					else{
						sR2.pos=a_read_2.bpos+1;
						sR.pos=max(long(a_art.ref_seq.size()-(a_read.bpos+read_len-1)), sR2.pos) ;
						sR.reverse_comp();
					...
					sR.pNext=sR2.pos;
					sR2.pNext=sR.pos;
					// technically template length is distance from leftmost base to rightmost base.
					// Here, we use the distance from the actual sequence, not the reference.
					// If there are indels wrt reference, then adjust the template length accordingly.
					if(sR2.pos>sR.pos){
						// Don't include softclipped adapter contamination in the template length
						// But do adjust template length for indels wrt reference
						sR.tLen=sR2.pos+a_read_2.getAdjustedSeqRefLen()-sR.pos;
						sR2.tLen=-sR.tLen;
					}
					else{
						sR2.tLen=sR.pos+a_read.getAdjustedSeqRefLen()-sR2.pos;
						sR.tLen=-sR2.tLen;
					}
					...
					if(err_free_sam){
						...
						if(a_read.is_plus_strand) {
							if (a_read.seq_ref.size() < read_len) {  // fragment shorter than read length?
								ostringstream oss;
								oss << a_read.seq_ref.size() << '=' << (read_len - a_read.seq_ref.size()) << 'H';
								sR.cigar = oss.str();
							}
							else {
								sR.cigar=p_cigar;
							...
							sR2.reverse_comp();
							if (a_read_2.seq_ref.size() < read_len) {  // fragment shorter than read length?
								ostringstream oss;
								oss << (read_len - a_read_2.seq_ref.size()) << 'H' << a_read_2.seq_ref.size() << '=';
								sR2.cigar = oss.str();
							}
							else {
								sR2.cigar=p_cigar;
							...
							if (a_read.seq_ref.size() < read_len) {  // fragment shorter than read length?
								ostringstream oss;
								oss << (read_len - a_read.seq_ref.size()) << 'H' << a_read.seq_ref.size() << '=';
								sR.cigar = oss.str();
							}
							else {
								sR.cigar=p_cigar;
							...
							if (a_read_2.seq_ref.size() < read_len) {
								ostringstream oss;
								oss << a_read_2.seq_ref.size() << '=' << (read_len - a_read_2.seq_ref.size()) << 'H';
								sR2.cigar = oss.str();
							}
							else {
								sR2.cigar=p_cigar;
